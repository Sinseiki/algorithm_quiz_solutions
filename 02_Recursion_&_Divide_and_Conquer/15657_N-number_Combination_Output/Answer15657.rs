fn main(){
  ...
  let line1_vec:Vec<u16> = line1.split_whitespace().map(|x| x.parse().unwrap()).collect();
  let total_length = line1_vec[0];
  let target_length = line1_vec[1];

  let mut line2 = String::new();
  std::io::stdin().read_line(&mut line2).unwrap();
  let mut sorted_numbers:Vec<u16> = line2.split_whitespace().map(|x| x.parse().unwrap()).collect();
  sorted_numbers.sort();

  let mut result_sequences = String::new();
  let mut sequence:Vec<u16> = Vec::new();

  fn dfs(start:usize, depth:u16, sorted_numbers:&[u16], total_length:u16, target_length:u16, result_sequences:&mut String, sequence:&mut Vec<u16>){
    ...
    for i in start .. total_length as usize {
